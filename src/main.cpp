...
void SensorTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        float degPerSec = gyroEvent.gyro.z * 57.2958f;
        data.yawRate = (int16_t)(degPerSec * 10);

        // TODO: 나머지 센서(coolantTemp, rpm, sus*, gps, speed, tps, gear) 채워기
        // data.rpm = kwp.sendRequest();
...
void DisplayTask(void *pvParameters) {
    ...
    for (;;) {
        if (xQueueReceive(sensorQueue, &latest, portMAX_DELAY) == pdTRUE) {
            // TM1637 기어 + 냉각수 표시
            display.showNumberDec(latest.gear * 1000 + latest.coolantTemp);
            ...
            if (rpm >= 8500) {
                ...
                for (int i = 0; i < active; i++) {
                    if (i < 5)        leds.setPixelColor(i, leds.Color(0, 255, 0));
                    else if (i < 10)  leds.setPixelColor(i, leds.Color(255, 200, 0));
                    else              leds.setPixelColor(i, leds.Color(255, 0, 0));
                }
            }
            leds.show();
        }
