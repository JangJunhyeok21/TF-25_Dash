...
// --- 전역 변수 ---
// RPM 측정을 위한 펄스 간 시간 간격 저장
volatile unsigned long lastPulseTime = 0;
volatile unsigned long pulseInterval = 0; // 마이크로초 단위
...
void setup()
{
    ...
    char dirPath[12];
    sprintf(dirPath, "/%04d%02d%02d", now.year(), now.month(), now.day());
    ...
    {
        // CSV 헤더 작성
        dataFile.println("Timestamp,AccelX,AccelY,AccelZ,GyroX,GyroY,GyroZ,RPM,OilTemp,Latitude,Longitude,Speed");
        dataFile.close();
...
void SensorTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        {
            DateTime gpsUtc(gps.date.year(), gps.date.month(), gps.date.day(),
                            gps.time.hour(), gps.time.minute(), gps.time.second());
            // RTClib의 TimeSpan을 이용해 UTC+9 (KST) 변환
            DateTime kst = gpsUtc + TimeSpan(0, 9, 0, 0);
            rtc.adjust(kst);
        ...
        {
            // RPM = (60,000,000 μs/분) / (interval μs/펄스) / pulsesPerRevolution
            rpmRaw = (uint16_t)((60000000UL / interval) / pulsesPerRevolution);
            // 비현실적인 고RPM 필터링 (20000 RPM 초과 시 무시)
            if (rpmRaw > 20000)
                rpmRaw = 0;
        }

        // 이동평균 필터 (5샘플)
        rpmBuffer[rpmIndex] = rpmRaw;
        rpmIndex = (rpmIndex + 1) % 5;
        if (rpmCount < 5)
            rpmCount++;
        uint32_t rpmSum = 0;
        for (uint8_t i = 0; i < rpmCount; i++)
            rpmSum += rpmBuffer[i];
        data.rpm = rpmSum / rpmCount;
        ...
        if (++tempLoopCounter >= 10)
        {
            ...
            {
                // NTC가 GND(하단)에 있을 때의 저항 계산
                float resistance = FIXED_RESISTOR * ((float)adcValue / (4095.0f - (float)adcValue));
                float steinhart = log(resistance / NOMINAL_RESISTANCE) / B_COEFFICIENT + 1.0f / (NOMINAL_TEMPERATURE + 273.15f);
                data.oilTemp = (uint16_t)((1.0f / steinhart) - 273.15f);
            }
            else
            {
                data.oilTemp = 0;
